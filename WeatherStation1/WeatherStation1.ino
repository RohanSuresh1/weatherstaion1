...
static void generateTelemetryPayload()
{
  ...
  float temperature = bme.readTemperature(); // Read temperature in degrees Celsius
  float humidity = bme.readHumidity();       // Read relative humidity as a percentage
  float pressure = bme.readPressure() / 100.0F; // Read pressure in hPa (hectopascals)
  float air_quality = gasSensor.getPPM(); //read MQ135 
  StaticJsonDocument<128> jsonDocument;

  // Add telemetry data to the JSON object
  jsonDocument["msgCount"] = telemetry_send_count++;
  jsonDocument["weatherStationId"] = 1;
  jsonDocument["weatherStationCode"] = "MCA_STN";
  jsonDocument["temperature"] = bme.readTemperature();
  jsonDocument["humidity"] = bme.readHumidity();
  jsonDocument["airQuality"] = air_quality;
  jsonDocument["contentType"] = "application/json";

  // Get the current time in UTC
  time_t utcTime = now();                                  // Get the current time in UTC

  // Define the time zone offset for IST (Indian Standard Time) in seconds
  int timeZoneOffset = 5 * 60 * 60 + 30 * 60;               // IST is UTC+5:30

  // Create a Timezone object with the IST offset
  TimeChangeRule myTZ;
  Timezone istTZ(myTZ);

  // Convert UTC time to IST
  time_t istTime = istTZ.toLocal(utcTime);

  // Format the time string
  char timeString[9];                                       // HH:MM:SS format
  sprintf(timeString, "%02d:%02d:%02d", hour(istTime), minute(istTime), second(istTime));

  // Add the time to the JSON object
  jsonDocument["recordeddate"] = timeString;

  // Serialize the JSON object to a string
  telemetry_payload = "";
  serializeJson(jsonDocument, telemetry_payload);
}
